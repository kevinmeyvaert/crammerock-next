...
import { Card, CardBody, CardImage, CardTitle } from '@/components/homepage/HomeContent';
      ...
      <MainGrid.Grid>
        ...
        <MainGrid.Main>
          ...
          {children ? (
            <ContentGrid>
              {children.map((child) => (
                <Card type="c" key={child.slug} link={`/${child.slug}`}>
                  {console.log(child)}
                  {child.featuredImage?.thumbnail && (
                    <CardImage data={child.featuredImage?.thumbnail} />
                  )}
                  <CardBody>
